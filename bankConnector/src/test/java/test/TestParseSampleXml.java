...
public class TestParseSampleXml extends DefaultHandler {
	...
	private void handleElement(Element ele, Stack<String> pathStack) {
		...
		if (children.size() > 0) {
			...
			if (isWithValue) {
				...
				vector.add(elementPath + ".value" + "=" + value);
			} else {
				parsed.put(elementPath, value);
				vector.add(elementPath + "=" + value);
			}
	...
	public void parseXml() {
		...
		try {
			...
			StringBuffer oLines = new StringBuffer();
			for(int i=0;i<vector.size();i++){
				oLines.append("#["+(i+1)+"]\r\n");
				oLines.append(vector.get(i).toString());
				oLines.append("\r\n\r\n");
			}
			//out.write(outLines.toString());
			out.write(oLines.toString());
			out.flush();
	...
	public static void main(String[] args) {
		...
		String inPath = "E:/Docs/techDocs/CNAPS2/人行文档/CNAPS2/互联规范(ver 1.1)/报文样例/ibps.101.001.01.xml";
